...
# 시장 RSI 증감율 체크
def check_RSI_rateofchange():
    result = 0
    for curCoin in CoinInfo :
        if round(CoinInfo[curCoin]["RSIBef"]) > round(CoinInfo[curCoin]["RSINow"]) :
            result -= 1
        elif round(CoinInfo[curCoin]["RSIBef"]) < round(CoinInfo[curCoin]["RSINow"]) :
            result += 1
    return result
...
def check_running_right() :
    message = "Now Monitoring... " + str(check_RSI_rateofchange())
    Prt_and_Slack(message)
    ...
    try :
        ...
        if Market_RSI == Market_RSI_Bef :
            ...
            if Market_RSI_Count > 50 :
                ...
                if Market_RSI_Bef == "Up_Market_RSI" : K_Value = 0.5
                elif Market_RSI_Bef == "Down_Market_RSI" : K_Value = 0.3
                Prt_and_Slack(message) 
        ...
        for curCoin in coinlist :  
            ...
            #조정 변수 불러오기
            target_price = get_target_price(curCoin, K_Value)
            ...
            if CoinInfo[curCoin]["CoinPriceMyBuy"] == 0:
                ...
                if target_price < current_price and ma15 < current_price and now_rsi >= CoinInfo[curCoin]["RSIBef"]:
                    ...
                    if check_RSI_rateofchange() >= 3 and len(TargetTouchCoin_Count) >= 2:
                        
                        # 구매 추천 정도(Very Strongly ~ Weakly)에 따라 Satisfied_Coin에 등록
            ...
            # 매도 조건 확인, 조건 달성 시 바로 매도 진행
                ...
                if Nowtime <= CoinInfo[curCoin]["BuyTime"]  + datetime.timedelta(days = 1) :
                    ...
                    elif CoinInfo[curCoin]["RSIHighest"] - 5 > now_rsi and CoinInfo[curCoin]["higher"] == True:
                        sell(curCoin,current_price)
                        print("By RSI over 70 when: ", now_rsi)
                        
                    # RSI가 70까지 도달 못하고 가격이 떨어질 때
                    elif CoinInfo[curCoin]["RSIHighest"] - 7 > now_rsi and CoinInfo[curCoin]["higher"] == False:
                        sell(curCoin,current_price)
