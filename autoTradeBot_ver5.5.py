...
def buy(coin): 
    ...
    if left > 0:
        upbit.buy_market_order(coin, 6000)
        time.sleep(0.2)
        CoinInfo[coin]["CoinPriceMyBuy"] += 6000
        CoinInfo[coin]["BuyTime"] = datetime.datetime.now()
        CoinInfo[curCoin]["lower"] = False
        left -= 6000
        message = coin + " buy " + str(CoinInfo[coin]["CoinPriceMyBuy"]) + "won " + str(left) + "left. Time: " + str(CoinInfo[coin]["BuyTime"])
...
#5일 이동 평균선 조회
def get_ma15(ticker):
    df = pyupbit.get_ohlcv(ticker, interval=interval_time, count=30)
    time.sleep(0.2)
    ma15 = df['close'].rolling(30).mean().iloc[-1]
    return ma15
...
# 슬랙 토큰 입력
myToken = "xoxb-3135357024800-3124236991793-o62B91PSRRWwJZ8NBlb0e6OH"
    ...
    try :
        ...
        for curCoin in coinlist :  
            ...
            # 매도 조건 확인, 조건 달성 시 바로 매도 진행
                ...
                # 구매 후 하루가 지나지 않았을 때
                if datetime.datetime.now() <= CoinInfo[curCoin]["BuyTime"]  + datetime.timedelta(days = 1) :
